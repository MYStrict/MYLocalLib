...
#pragma mark - EDIT BY MQ
- (void) ymShowWithView : (UIView *) view
         withCompletion : (FSActionSheetHandler)handler {
    ...
    [view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|[_backView]|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(_backView)]];
    [view addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|[_backView]|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(_backView)]];
    
    [self.tableView reloadData];
    // 内容高度
    CGFloat contentHeight = self.tableView.contentSize.height;
    // 适配屏幕高度
    CGFloat contentMaxHeight = CGRectGetHeight(view.frame)*FSActionSheetContentMaxScale;
    if (contentHeight > contentMaxHeight) {
        self.tableView.scrollEnabled = YES;
        contentHeight = contentMaxHeight;
    }
    ...
    [UIView animateWithDuration:0.25 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        pSelf.backView.alpha   = 0.38;
        CGRect newFrame   = pSelf.frame;
        newFrame.origin.y = CGRectGetMaxY(view.frame)-selfH;
        pSelf.frame        = newFrame;
    } completion:^(BOOL finished) {
        // constraint
        ...
        pSelf.heightConstraint = [NSLayoutConstraint constraintWithItem:pSelf attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1 constant:contentHeight];
        [view addConstraint:pSelf.heightConstraint];
    }];
}
