...
#pragma mark - EDIT BY MINGQING
        _placeholderImage = [UIImage myGenerateImageWithColor:[UIColor whiteColor] withSize:100.f];
    }
...
- (void)longPress:(UILongPressGestureRecognizer *)longPress
{
    ...
    if (longPress.state == UIGestureRecognizerStateBegan) {
         ...
         */
#pragma mark - EDIT BY MQ
        [actionSheet ymShowWithView:self withCompletion:^(NSInteger selectedIndex) {
            if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(photoBrowser:clickActionSheetIndex:currentImageIndex:)]) {
                [weakSelf.delegate photoBrowser:weakSelf clickActionSheetIndex:selectedIndex currentImageIndex:weakSelf.currentImageIndex];
            }
        }];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;
{
    ...
    [self addSubview:self.savaImageTipLabel];
#pragma mark - EDIT BY MQ
//    [MBProgressHUD ymShowMessage:ymLocalize(error ? @"_YM_SAVE_FAIL_" : @"_YM_SAVE_SUCCESS_", "保存失败 / 成功")];
    if (error) {
